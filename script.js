 ...
 */
  ...
  */
function start() {
  alert("Leikurinn gengur út á að giska á tölu milli 0 - 100, þangað til að þú færð út rétt");
  var another = true;
  while(another){
    var leikur = play();
    another = confirm('Viltu spila annan?');
  };
...
function play() {
  ...
  while(true){
    ...
    var guess = prompt("Giskaðu á tölu");
    if(guess === null)
      {
      alert("Hætt í leik");
      break;
    ...
    guessCounter++;
    var guessInt = parseGuess(guess);
    var response = getResponse(guessInt,random);
    alert(response);
    if(response === 'Rétt!')
    {
      games.push(guessCounter);
      break;
    }
 ...
 */
function getResults(){
  if(games === []){return 'Þú spilaðir engann leik >_<'}
  var leikir = games.length;
  
  return ("þú spilaðir "+leikir+" leiki\nMeðalfjöldi ágiskana var "+calculateAverage());
}
...
function getResponse(guess, correct){
  if(guess < 0 || isNaN(guess)){return 'Ekki rétt'};
  if(Math.abs(guess - correct) === 0){return 'Rétt!'};
